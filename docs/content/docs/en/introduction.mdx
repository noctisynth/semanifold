...
<Availability status="next">
  The next release adds repository-local JavaScript ecosystem plugins. A plugin implements the same discovery, inspection, and edit-planning boundary, so custom packages join the same graph instead of living in a separate release script.
</Availability>
