...
<Availability status="next">
  The plugin runtime is complete on `main` and is planned for the next Semifold release after `0.3.0-rc.5`. The current manifests are `semifold 0.3.0-rc.6` and `@semifold/plugin-sdk 0.1.0-rc.0`, but no matching release tag exists yet.
</Availability>
