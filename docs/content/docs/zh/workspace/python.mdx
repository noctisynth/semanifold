...
当前内置 Python 发现不会从项目元数据推断“私有软件包”，发现的软件包都会被视为可发布。请只把真正需要进入发布流程的 Python 项目保留在配置中；如果自定义格式带有私有标记，应由插件在发现结果中明确返回。GitHub Release 是否创建仍由软件包级 `github-release` 策略独立控制。
