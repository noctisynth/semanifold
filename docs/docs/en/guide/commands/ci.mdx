...
- Commit message: configurable with `[release].commit-message`; defaults to `chore(release): bump versions`
- Force-pushes to ensure consistency

### Pull Request

- Creates PR from release branch to base branch
- PR title: configurable with `[release].pull-request-title`; defaults to `chore(release): bump versions`
