...
# Optional; omit to keep the built-in messages.
[release]
...
Optionally configures strict MiniJinja templates for release automation. Both fields expose the same `release.*` context as the release branch template and default to `chore(release): bump versions` when omitted. `smif init` and `smif config sync` do not add this section automatically.

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `commit-message` | string | `"chore(release): bump versions"` | Release branch commit message; multiline output is allowed |
| `pull-request-title` | string | `"chore(release): bump versions"` | Release pull request title; output must be one non-empty line |
...
| `github-release` | boolean | No | Override GitHub Release creation for this package; defaults to enabled for public packages and disabled for private packages |
