...
可选地配置发布自动化使用的严格 MiniJinja 模板。两个字段暴露与发布分支模板相同的 `release.*` 上下文，省略时均默认使用 `chore(release): bump versions`。`smif init` 和 `smif config sync` 不会自动加入这个分区。

| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `commit-message` | string | `"chore(release): bump versions"` | 发布分支的提交消息；允许多行输出 |
| `pull-request-title` | string | `"chore(release): bump versions"` | 发布 Pull Request 标题；输出必须是非空单行文本 |
...
| `github-release` | boolean | 否 | 覆盖该包的 GitHub Release 创建策略；public 包默认启用，private 包默认禁用 |
